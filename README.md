...
1. `MAAZ_ztelier_v5_1_0.html`（または `index.html`）をダウンロードする
2. ブラウザでファイルを開く
...
- アプリケーション: **v5.1.0**（HTML 内の `const APP_VER` が唯一の版数の出所）
...
- **v5.1.0** — 18 コントロールそれぞれに **AI 利用に対する成熟度基準**を追加（④記入の達成基準リストと CISO Executive Report §4 に「AI」バッジ付き 1 行で併記。既存の成熟度レベル・達成基準は無改変で、表示専用・採点には関与しない）。あわせて §4 ロードマップの改ページ算出を、固定行高の定数から達成基準の実文字数に基づく実測行高へ変更し、AI 基準の併記でアクション行が伸びた際の紙面溢れを解消した。**評価ロジック・スコア・エクスポート形式（`maaz-v4.4.0` / `maaz-v4-csfa`）に変更はない**（AI 成熟度基準はエクスポート JSON には含まれません）
