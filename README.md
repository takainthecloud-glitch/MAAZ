...
- **Action Plan** — Quick Win（0–3ヶ月）/ Short-term（3–6ヶ月）/ Strategic（6–12ヶ月）の三層で施策を優先度つきに整理。各コントロールに対応するソリューションカテゴリ（IDaaS、EDR、DLP、XDR、SOAR 等の一般名称）を併記
- **CISO Executive Report** — A4 縦 9 ページ構成（表紙 / サマリ / 現状評価 / ピラー別改善内訳 / 脅威ランドスケープ / ロードマップ / 投資提言 / 付録）を印刷・PDF 保存
- **入出力** — 評価の保存・再開と下流ツール連携を兼ねた 4 系統
  - **ROIツール連携 (JSON)** — 評価全体（AS-IS / TO-BE、脆弱性係数、脅威カバレッジ、アクション）を書き出す標準形式。**JSONインポート**でそのまま読み戻して続きから再開できる
...
1. `MAAZ_ztelier_v3_2_0.html`（または `index.html`）をダウンロードする
2. ブラウザでファイルを開く
...
- アプリケーション: **v3.2.0**（HTML 内の `const APP_VER` が唯一の版数の出所）
