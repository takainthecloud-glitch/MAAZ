...
- **Mapping（マッピング透明性）** — ZTMM コントロール ↔ MITRE 技法の対応表そのものを画面上で公開する章。各行について「どのレベルで」「どの重み（防止 / 検知 / 可視化）で」効くのかと、その根拠を示す。どの統制からも到達しない技法（orphan）や、そのレベルでは新規緩和が発生しない経路も明示するため、スコアの出どころを外部から検証できる
...
1. `MAAZ_ztelier_v3_7_1.html`（または `index.html`）をダウンロードする
2. ブラウザでファイルを開く
...
- アプリケーション: **v3.7.1**（HTML 内の `const APP_VER` が唯一の版数の出所）
